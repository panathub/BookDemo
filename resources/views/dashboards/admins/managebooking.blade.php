			...
			<div class="card-body">
				<div class="table-responsive">
					<table class="table table-condensed align-items-center" id="bookings-table">
						<thead>
							<tr>
								<th><input type="checkbox" name="booking_main_checkbox"><label></label></th>
								<th>#</th>
								<th>ห้อง</th>
								<th>ผู้จอง</th>
								<th>แผนก</th>
								...
								<th>สถานะ</th>
								<th>รายละเอียด</th>
								<th>Action<button class="btn btn-sm btn-danger d-none" id="deleteAllBookingBtn">Delete ALL</button></th>
							</tr>
						</thead>
						<tbody>
						</tbody>
...
<!-------------------------------Modal Report Info ------------------------------------------>
<div class="modal fade infoReport" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true" data-keyboard="true">
	<div class="modal-dialog modal-dialog-centered" role="document">
		<div class="modal-content">
			<div class="modal-header pb-0">
...
<script>
			...
			$.ajax({
				...
				success: function(data) {
					...
					if (data.code == 0) {
						...
						$(".button").attr("disabled", false);
						$(".btn-txt").text("บันทึกการจองห้องประชุม");
						$('#request-bookings-table').DataTable().ajax.reload(null, false);
	...
	$(document).on('click', 'input[name="booking_main_checkbox"]', function() {
		if (this.checked) {
			$('input[name="booking_checkbox"]').each(function() {
				this.checked = true;
			});
	...
	function toggledeleteAllBookingBtn() {
		if ($('input[name="booking_checkbox"]:checked').length > 0) {
			$('button#deleteAllBookingBtn').text('Delete (' + $('input[name="booking_checkbox"]:checked').length + ')').removeClass('d-none');
		} else {
			$('button#deleteAllBookingBtn').addClass('<br> d-none');
		}
	}

	$(document).on('click', 'button#deleteAllBookingBtn', function() {
		var checkedBooking = [];
		$('input[name="booking_checkbox"]:checked').each(function() {
			checkedBooking.push($(this).data('id'));
		});
		var url = '{{ route("delete.selected.booking") }}';
		if (checkedBooking.length > 0) {
			Swal.fire({
				title: 'Are you sure?',
				html: 'You want to delete <b>(' + checkedBooking.length + ')</b> bookings',
				icon: 'warning',
				showCancelButton: true,
				showCloseButton: true,
				confirmButtonText: 'Yes, Delete',
				...
				if (result.value) {
					$.post(url, {
						booking_id: checkedBooking
					}, function(data) {
						if (data.code == 1) {
							$('#bookings-table').DataTable().ajax.reload(null, true);
							Swal.fire({
								position: 'top-end',
								icon: 'success',
								title: (data.msg),
