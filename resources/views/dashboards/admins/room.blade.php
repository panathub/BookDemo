...
<div class="container-fluid mt--6">
    ...
    <div class="modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content">
            ...
            <!--Modal Info -->
    <div class="modal fade infoRoom" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true" data-keyboard="false" data-backdrop="static">
    <div class="modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content">
            <div class="modal-header">
...
$('#addroom').on('submit', function(e){
      ...
      $(document).on('click','#editRoomBtn', function(){ 
         ...
         $.post('<?= route("get.room.details") ?>',{room_id:room_id}, function(data){
                ...
                $(document).on('click','#deleteRoomBtn', function(){
                    ...
                    swal.fire({
                         ...
                         allowOutsideClick:false
                          ...
                          if(result.value){
                              $.post(url,{room_id:room_id}, function(data){
                                   if(data.code == 1){
                                       $('#rooms-table').DataTable().ajax.reload(null, false);
                                       Swal.fire({
                                    position: 'top-end',
            ...
            $('input[type="file"][name="Image_room"]').on('change', function(){
                ...
                if(extension == 'jpeg' || extension == 'jpg' || extension == 'png'){
                     if(typeof(FileReader) != 'undefined'){
                          ...
                          var reader = new FileReader();
                          reader.onload = function(e){
                              $('<img/>',{'src':e.target.result,'class':'img-fluid','style':'max-width:10;margin-bottom:10px;'}).appendTo(img_holder);
                          }
                          img_holder.show();
                          reader.readAsDataURL($(this)[0].files[0]);
                     }else{
